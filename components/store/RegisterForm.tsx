...
export default function RegisterForm() {
  const [state, formAction, isPending] = useActionState<AuthFormState, FormData>(registerCustomer, {});
  const err = state.errors ?? {};
  return (
    <form action={formAction} className="space-y-4">
      {state.message && state.message !== 'ok' && (
        <p className="rounded-lg px-3 py-2 text-sm" style={{ background: 'var(--error-bg)', color: 'var(--error-text)' }}>{state.message}</p>
      )}
      <div className="space-y-1.5">
        <label className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>Full Name</label>
        <input name="name" type="text" autoComplete="name" required className="input-theme w-full px-3 py-2.5 text-sm" placeholder="John Smith" />
      ...
      <div className="space-y-1.5">
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Min. 8 characters</span>
        </div>
        <input name="password" type="password" autoComplete="new-password" required className="input-theme w-full px-3 py-2.5 text-sm" />
